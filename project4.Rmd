...
__Plotting Daily dates by time. __
...
__Performing percentage distribution in death rate__
we use daily % over total that date to distribute the rate proportionally. 
...
plot(data$Date,
  ...
  ylab = "Daily Deaths")
```
Partitioning the data as testing and training data.
...
```
__Double Quadratic Linear Regression__
```{r}
...
```
__Fifth order linear regression model. __
...
plot(new_deaths_total ~ SN, 
     ...
     xlab = "Date",
     ylab = "Cumulative Deaths")
abline(knnreg(new_deaths_total ~ SN, data = train.data), col = "green", lwd=2)
```


__ANN-MLP regression model with 2 hidden layers with 3 and 2 neurons__
...
plot(ann)
```
__Select the best model with lowest RMSE on the test data.__
...
__Write a summary and recommendation for Ministry of Health, Nepal.__
