...
mtcar <- as.data.frame(mtcars)
stem(mtcar$mpg)

ggplot(mtcars,aes(mpg))+
  geom_histogram(col="steelblue")+
  labs(x="mpg",y="Count",title = "Histogram of mpg")
...
Ho: The distribution of mpg data is not significantly different from normal distribution.
H1: The distribution of mpg data is significantly different from normal distribution

shapiro.test(mtcar$mpg)

Here p-value > 0.05
Therefore, Ho is accepted.
Hence , the distribution is normal.

Here the sample size is less than 50 so, Shapiro-Wilk test is perfomed. For sample size more than 50 Kolmogorov-Smirnov is 
carried out.